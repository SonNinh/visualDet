...
def main(config="config/config.py", experiment_name="default", world_size=1, local_rank=-1):
    ...
    for epoch_num in range(cfg.trainer.max_epochs):
        ...
        for data in tqdm_bar:
            iter_num += 1
            training_dection(data, detector, optimizer, training_loss_logger, cfg)
            ...
            log_str = f'Train at Epoch: {epoch_num} | Iter: {iter_num}  | Running loss: {total_loss_avg:2.5f} | eta:{eta}'
