...
def get_label_annos_pickle(label_folder):
    all_annos = []
    

    imdb_file_path = os.path.join(label_folder, 'imdb.pkl')
    imdb = pickle.load(open(imdb_file_path, 'rb')) # list of kittiData
    
    for idx in range(len(imdb)):
        annotations = {}
        content = imdb[idx].label
        annotations['name'] = np.array([x.type for x in content])
        annotations['truncated'] = np.array([x.truncated for x in content])
        annotations['occluded'] = np.array([x.occluded for x in content])
        annotations['alpha'] = np.array([x.alpha for x in content])
        annotations['bbox'] = np.array(
            [[x.bbox_l, x.bbox_t, x.bbox_r, x.bbox_b] for x in content]
        ).reshape(-1, 4)
        # dimensions will convert hwl format to standard lhw(camera) format.
        annotations['dimensions'] = np.array(
            [[x.h, x.w, x.l] for x in content]
        ).reshape(-1, 3)[:, [2, 0, 1]]
        annotations['location'] = np.array(
            [[x.x, x.y, x.z] for x in content]
        ).reshape(-1, 3)
        annotations['rotation_y'] = np.array(
            [x.ry for x in content]
        ).reshape(-1)
