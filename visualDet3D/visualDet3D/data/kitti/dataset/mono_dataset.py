...
class NuscenesMonoDataset(torch.utils.data.Dataset):
    """Some Information about KittiDataset"""
    def __init__(self, cfg, split='training'):
        super(NuscenesMonoDataset, self).__init__()
        preprocessed_path   = cfg.path.preprocessed_path
        obj_types           = cfg.obj_types
        is_train = (split == 'training')
        ...
        self.is_train = is_train
        self.obj_types = obj_types
        self.use_right_image = getattr(cfg.data, 'use_right_image', False)
        self.is_reproject = getattr(cfg.data, 'is_reproject', True) # if reproject 2d
    ...
    def __getitem__(self, index):
        nuscenes_data = self.imdb[index % len(self.imdb)]
        # The calib and label has been preloaded to minimize the time in each indexing
        if nuscenes_data is None:
            print(index, len(self))
        nuscenes_data.preprocess_phase = False
        
        nuscenes_data.output_dict = self.output_dict
        _, image, _ = nuscenes_data.read_data()
        calib = nuscenes_data.calib
        calib.image_shape = image.shape

        label = nuscenes_data.label # label: list of kittiObj
        label = []
        for obj in nuscenes_data.label:
            if obj.type in self.obj_types:
                label.append(obj)
        transformed_image, transformed_P2, transformed_label = self.transform(
            image, p2=deepcopy(calib.P2), labels=deepcopy(label)
        )
        bbox3d_state = np.zeros([len(transformed_label), 7]) #[camera_x, camera_y, z, w, h, l, alpha]
        if len(transformed_label) > 0:
            transformed_label, bbox3d_state = self._reproject(transformed_P2, transformed_label)
        ...
        output_dict = {'calib': transformed_P2,
                       ...
                       'original_shape':image.shape,
                       'original_P':calib.P2.copy(),
                       'sample_token': nuscenes_data.sample_token,
                       'image_id': nuscenes_data.image_id}
        return output_dict
...
@DATASET_DICT.register_module
class NuscenesMonoTestDataset(KittiMonoDataset):
    def __init__(self, cfg, split='test'):
        preprocessed_path   = cfg.path.preprocessed_path
        obj_types           = cfg.obj_types
        super(NuscenesMonoTestDataset, self).__init__(cfg, 'test')
        is_train = (split == 'training')
        imdb_file_path = os.path.join(preprocessed_path, 'test', 'imdb.pkl')
        self.imdb = pickle.load(open(imdb_file_path, 'rb')) # list of kittiData
        self.output_dict = {
