...
class NuscenesLabel:
    ...
    class storing KITTI 3d object detection label
        self.data ([KittiObj])
    '''
    def __init__(self, anno_data, categories):
        
        self.data = [NuscenesObj(anno, categories) for anno in anno_data]
...
class NuscenesObj():
    ...
    class storing a KITTI 3d object
    '''
    def __init__(self, anno, categories):
        self.type = categories[anno['category_id']]['name']
        self.truncated = anno['truncated']
        self.occluded = anno['occluded']
        self.alpha = anno['alpha']
        self.bbox_l, self.bbox_t, w, h = anno['bbox']
        self.bbox_r, self.bbox_b = self.bbox_l + w, self.bbox_t + h
        self.h, self.w, self.l = anno['dim']
        self.x, self.y, self.z = anno['location']
        self.ry = anno['rotation_y']
        self.score = None
...
class NuscenesData:
    '''
    class storing a frame of Nuscenes data
    '''
    def __init__(self, root_dir, image_data, anno_data, categories, output_dict=None, preprocess_phase=False):
   
        self.sample_token = image_data['sample_token']
        self.image_id = image_data['id']
        self.image2_path = os.path.join(root_dir, image_data["file_name"])

        self.calib = NuscenesCalib(image_data['calib'])

        self.label = NuscenesLabel(anno_data, categories)
        ...
        self.preprocess_phase = preprocess_phase

    def read_data(self):
        if not self.preprocess_phase:
            image = read_image(self.image2_path)
        else:
            image = self.image2_path

        return self.calib, image, self.label
