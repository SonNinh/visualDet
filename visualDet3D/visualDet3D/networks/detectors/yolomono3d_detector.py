...
class Yolo3D(nn.Module):
    ...
    def test_forward(self, img_batch, P2, is_warming_up=False):
        """
        Args:
            img_batch: [B, C, H, W] tensor
            calib: visualDet3D.kitti.data.kitti.KittiCalib or anything with obj.P2
        ...
        if not is_warming_up:
            ...
            self.time_elapsed['feed_forward'] += curr_time
        ...
        if not is_warming_up:
            ...
            torch.cuda.synchronize()
            curr_time = self.starter.elapsed_time(self.ender)
            self.time_elapsed['post_process'] += curr_time

        return scores, bboxes, cls_indexes
    
    def test_forward_batch(self, img_batch, P2):
        """
        Args:
            img_batch: [B, C, H, W] tensor
            calib: visualDet3D.kitti.data.kitti.KittiCalib or anything with obj.P2
        ...
        scores, bboxes, cls_indexes = self.bbox_head.get_bboxes_batch(cls_preds, reg_preds, anchors, P2, img_batch)

        return scores, bboxes, cls_indexes
    
    

    def forward(self, inputs, is_warming_up=False):
        ...
        if isinstance(inputs, list) and len(inputs) == 3:
            ...
            return self.test_forward(img_batch, calib, is_warming_up=is_warming_up)
            # return self.test_forward_batch(img_batch, calib)
