...
class DCNAlternative(nn.Module):
...
class AnchorBasedDetection3DHead(nn.Module):
    ...
    def init_layers(self, num_features_in,
        ...
        self.reg_feature_extraction = nn.Sequential(
            ...
            DCNAlternative(num_features_in, reg_feature_size),
    ...
    def get_bboxes_batch(self, cls_scores, reg_preds, anchors, P2s, img_batch=None):
        batch_max_score = []
        batch_bboxes = []
        batch_label = []
        ...
        for j in range(batch_size):
            ...
            anchor_j      = anchor_j[high_score_mask, :]
            anchor_mean_std_3d_j = anchor_mean_std_3d_j[high_score_mask, :]
            cls_score   = cls_score[high_score_mask, :]
            alpha_score = alpha_score[high_score_mask, :]
            reg_pred    = reg_pred[high_score_mask, :]
            max_score   = max_score[high_score_mask]
            ...
            bboxes, mask = self._decode(anchor_j, reg_pred, anchor_mean_std_3d_j, label, alpha_score)
            if img_batch is not None:
                bboxes = self.clipper(bboxes, img_batch)
            cls_score = cls_score[mask]
            max_score = max_score[mask]
            ...
            batch_max_score.append(max_score)
            batch_bboxes.append(bboxes)
            batch_label.append(label)
            
            return batch_max_score, batch_bboxes, batch_label
