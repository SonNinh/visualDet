...
def load_prediction(
    result_path: str, max_boxes_per_sample: int, 
    box_cls, verbose: bool = False) -> Tuple[EvalBoxes, Dict]:
    """
...
def load_gt_json(gt_path: str, box_cls, valid_tokens, verbose: bool = False) -> EvalBoxes:
    ...
    for img_id, anno_data in groupby(nusc['annotations'], lambda k: k['image_id']):
        ...
        if str(img_id) in valid_tokens:
            sample_boxes = []
            # sample_token = nusc['images'][img_id]['sample_token']
            for anno in anno_data:
                sample_boxes.append(
                    box_cls(
                        sample_token=str(img_id),
                        translation=anno['location'],
                        size=anno['dim'],
                        rotation=anno['rotation_y'],
                        velocity=0.,
                        num_pts=-1,
                        detection_name=nusc['categories'][anno['category_id']]['name'],
                        detection_score=-1.0,  # GT samples do not have a score.
                        attribute_name=''
                    )
                )
            all_annotations.add_boxes(str(img_id), sample_boxes)
...
def my_add_center_dist(eval_boxes: EvalBoxes):
    """
    Adds the cylindrical (xy) center distance from ego vehicle to each box.
    :param nusc: The NuScenes instance.
    :param eval_boxes: A set of boxes, either GT or predictions.
    ...
    for sample_token in eval_boxes.sample_tokens:
        ...
        for box in eval_boxes[sample_token]:
            # Both boxes and ego pose are given in global coord system, so distance can be calculated directly.
            # Note that the z component of the ego pose is 0.
            ego_translation = (box.translation[0] - 0,
                               box.translation[1] - 0,
                               box.translation[2] - 0)
            if isinstance(box, DetectionBox) or isinstance(box, TrackingBox):
                box.ego_translation = ego_translation
...
def filter_eval_boxes_except_bike_rack(
                      eval_boxes: EvalBoxes,
